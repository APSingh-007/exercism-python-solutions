...
def approx_average_is_average(hand: list) -> bool:
    ...
    true_average: float = card_average(hand=hand)
    approx_avg_fl: float = (hand[0] + hand[-1]) / 2
    approx_avg_mid: float = hand[len(hand) // 2]
    return true_average in [approx_avg_fl or true_average, approx_avg_mid]
...
def average_even_is_average_odd(hand: list) -> bool:
    ...
    even_average: int = card_average(hand[1::2])
    odd_average: int = card_average(hand[::2])
